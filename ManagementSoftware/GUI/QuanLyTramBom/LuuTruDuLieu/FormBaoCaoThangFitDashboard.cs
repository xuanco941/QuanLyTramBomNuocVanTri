...
{
    public partial class FormBaoCaoThangFitDashboard : Form
    {
        public FormBaoCaoThangFitDashboard()
        {
            InitializeComponent();
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        try
                        {
                            using (var workBook = new XLWorkbook())
                            {
                                ...
                                ws.Cell("B42").Value = String.Format("{0:0.00}", Math.Round((trungBinhBeHut / list.Count), 2, MidpointRounding.AwayFromZero));
                                ws.Cell("C42").Value = String.Format("{0:0.00}", Math.Round((trungBinhBeXa / list.Count), 2, MidpointRounding.AwayFromZero));
                                ws.Cell("D41").Value = Math.Floor(TongThoiGianChayBom1 / 60).ToString() + " h " + (TongThoiGianChayBom1 % 60).ToString() + " min ";
                                ws.Cell("E41").Value = Math.Floor(TongThoiGianChayBom2 / 60).ToString() + " h " + (TongThoiGianChayBom2 % 60).ToString() + " min ";
                                ws.Cell("F41").Value = Math.Floor(TongThoiGianChayBom3 / 60).ToString() + " h " + (TongThoiGianChayBom3 % 60).ToString() + " min ";
                                ws.Cell("G41").Value = Math.Floor(TongThoiGianChayBom4 / 60).ToString() + " h " + (TongThoiGianChayBom4 % 60).ToString() + " min ";
        ...
        private void FormBaoCaoThangFitDashboard_Load(object sender, EventArgs e)
        {
